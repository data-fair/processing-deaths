...
async function updateInconsistency (tmpDir, pluginConfig, processingConfig, dataset, axios, log) {
  ...
  if (fs.existsSync(`${tmpDir}/fichier-opposition-deces.csv`)) {
    try {
      const filePath = `${tmpDir}/fichier-opposition-deces.csv`
  ...
  try {
    ...
    await log.error(`${err.status}, ${err.statusText} ${JSON.stringify(err.data.errors)}`)
  }
}

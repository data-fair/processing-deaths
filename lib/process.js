...
const newCom = async function (refCodeInseeComm, processingConfig, axios) {
      ...
      if (changement[elem.COM_AV]) {
        ...
        for (const com of changement[elem.COM_AV]) {
          if (com[0] === elem.COM_AP) {
            test = true
            if (com[1] < elem.DATE_EFF) {
              console.log(elem.COM_AV, elem.COM_AP, elem.DATE_EFF)
              changement[elem.COM_AV] = [[elem.COM_AP, elem.DATE_EFF]]
            }
          }
        }
  ...
  let pops = (await axios.get(`api/v1/datasets/${processingConfig.datasetPopulation.id}/lines?format=json`, { params: { size: 10000, select: 'population_municipale,code' } })).data
  const popComm = []
  for (const elem of pops.results) {
    popComm[elem.code] = elem.population_municipale
  }
  while (pops.results.length === 10000) {
    pops = (await axios.get(pops.next)).data
    for (const elem of pops.results) {
      popComm[elem.code] = elem.population_municipale
    }
  }
    ...
    if (changement[numero]) {
      ...
      for (const change of cheminActuel) {
        if (changements.length > 1) {
          const change2 = changements[0]
          if (change[0] === change2[0]) {
            test = false
            if (new Date(change[1]) < new Date(change2[1])) {
              cheminActuel = [...cheminActuel, [change2[0], change2[1]]]
      ...
      if (test) {
        if (changements.length > 1) {
          ...
          if (datesSimilaires) {
            let max = changements[0][0]
            let maxDate = changements[0][1]

            for (const [nouveauNumero, date] of changements) {
              // Check whether the new population is greater than the current maximum population
              if (popComm[nouveauNumero] > popComm[max]) {
                max = nouveauNumero
                maxDate = date
              }
            }
...
async function parseLines (lines, refCodeInseeComm, refCodeInseePays, keysRef, processingConfig, axios) {
  try {
    const newComs = await newCom(refCodeInseeComm, processingConfig, axios)
    const out = []
